...
If you find that the Loader time consumption of [MiniCssExtractPlugin](https://webpack.docschina.org/plugins/mini-css-extract-plugin#root) or [CssExtractRspackPlugin](https://rspack.dev/plugins/rspack/css-extract-rspack-plugin) is too long, it is because the Loader includes the time consumption and idle time of other Loaders, for the following reasons:
